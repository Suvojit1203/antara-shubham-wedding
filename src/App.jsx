...
import { CalendarDays, ChevronDown, Heart, MapPin, Menu, Music, Pause, X } from 'lucide-react';
...
function CelebrationEffects() { return <div className="celebration-effects" aria-hidden="true"><span className="rocket rocket-one">✦</span><span className="rocket rocket-two">✦</span><span className="crackle crackle-one">✦ ✧ ✦</span><span className="crackle crackle-two">✦ ✧ ✦</span><span className="pixel pixel-one"/><span className="pixel pixel-two"/><span className="pixel pixel-three"/><span className="pixel pixel-four"/></div>; }
...
function App() {
  const [opened, setOpened] = useState(false), [menu, setMenu] = useState(false), [playing, setPlaying] = useState(false), [progress, setProgress] = useState(0);
  ...
  return <>{wedding.music.src && <audio ref={audio} loop preload="none" src={wedding.music.src}/>}<div className="progress" style={{width:`${progress}%`}} />
    {!opened && <Opening onOpen={begin} onPlayMusic={startMusic}/>} {opened && <main className="site-enter">
      ...
      <section className="hero section" id="home"><CelebrationEffects/><div className="hanging-bells" aria-hidden="true">⌇❦⌇</div><div className="hero-couple"><img src="/images/opening-couple.jpeg" alt="Antara and Shubham"/></div><p className="eyebrow">We joyfully invite you to celebrate</p><h1>Antara <em>&</em><br/> Shubham</h1><Ornament/><p className="hero-copy">Begin a beautiful new chapter with us</p><div className="date-card"><CalendarDays size={20}/><div><b>{wedding.displayDate}</b><span>{wedding.day}</span></div></div><a href="#details" className="scroll-cue">Discover our celebration <ChevronDown size={16}/></a></section>
      <section className="section family" id="details"><p className="eyebrow">With the blessings of</p><h2>Two families, one celebration</h2><Ornament/><div className="family-grid family-grid-photo"><article><p className="side-label">Groom's family</p><h3>{wedding.groom.name}</h3><p>Son of</p><b>Mr. {wedding.groom.father}</b><b>Mrs. {wedding.groom.mother}</b></article><figure className="family-hands"><img src="/images/hands.jpeg" alt="The couple's hands together" loading="lazy" decoding="async"/></figure><article><p className="side-label">Bride's family</p><h3>{wedding.bride.name}</h3><p>Daughter of</p><b>Mr. {wedding.bride.father}</b><b>Mrs. {wedding.bride.mother}</b></article></div></section>
      <section className="section details"><p className="eyebrow">Mark your calendar</p><h2>Celebration dates</h2><Ornament/><p className="celebration-intro">Come celebrate every beautiful moment with us</p><div className="event-grid">{wedding.events.map(e=><article key={e.title}><i>{e.icon}</i><h3>{e.title}</h3><p>{e.date}</p><b>{e.time}</b></article>)}</div><p className="countdown-label">The countdown begins</p><Countdown/></section>
      <section className="section story" id="story"><div className="story-photo"><span className="story-pixels" aria-hidden="true">✦ · ✧ · ✦</span><img src="/images/couple-portrait.jpeg" alt="Antara and Shubham" loading="lazy" decoding="async"/></div><div><p className="eyebrow">Our story</p><h2>Written in the stars,<br/>celebrated with love</h2><Ornament/><p>With grateful hearts, we invite you to join our families as we celebrate a promise of forever. Your blessings and presence will make our day truly complete.</p><Heart size={24} fill="currentColor"/></div></section>
      <section className="section gallery" id="gallery"><p className="eyebrow">Moments to cherish</p><h2>Our gallery</h2><p className="gallery-quote">“Every love story is beautiful, but ours is our favourite.”</p><div className="gallery-grid">{wedding.photos.map(photo=><figure className={`photo ${photo.gradient}`} key={photo.label}><img src={photo.src} alt={photo.alt} loading="lazy" decoding="async"/><figcaption>{photo.label}</figcaption></figure>)}</div></section>
